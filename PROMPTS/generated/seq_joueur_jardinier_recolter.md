...
# PROMPT SPÉCIALISÉ — animation : le Jardinier
...
- Type d'entité `animation` : champs requis = id, entity_id, state_or_action, frames, fps, loop.
...
Séquence `recolter` de `joueur_jardinier` — 4 animations :
- `anim_joueur_jardinier_recolter_down` direction=down frames=4 fps=6 loop=False impact=3 effet=effet:recolte
- `anim_joueur_jardinier_recolter_up` direction=up frames=4 fps=6 loop=False impact=3 effet=effet:recolte
- `anim_joueur_jardinier_recolter_left` direction=left frames=4 fps=6 loop=False impact=3 effet=effet:recolte
- `anim_joueur_jardinier_recolter_right` direction=right frames=4 fps=6 loop=False impact=3 effet=effet:recolte

## 5. Relations dans le graphe du monde

- Relations entrantes (ce qui pointe vers l'entité) :
  - `fait_joueur_jardinier` —[decrit]→ **joueur_jardinier**
- Relations sortantes (ce vers quoi l'entité pointe) :
  - **joueur_jardinier** —[habite]→ `lieu_maison_racine`
  - **joueur_jardinier** —[releve_de]→ `system_progression`
...
Produis la **spécification de production** de l'animation `joueur_jardinier`
(action `recolter`, direction `toutes (séquence)`). L'animation doit être
...
```json
{
  ...
  "state_or_action": "recolter",
  "direction": "toutes (séquence)",
