...
- Relations entrantes (ce qui pointe vers l'entité) :
  - `system_relations` —[utilise]→ **pnj_alba**
- Relations sortantes (ce vers quoi l'entité pointe) :
  - **pnj_alba** —[habite]→ `lieu_maison_racine`
  - **pnj_alba** —[frequente]→ `lieu_maison_racine`
  - **pnj_alba** —[frequente]→ `lieu_champ_de_la_vallee_claire`
  - **pnj_alba** —[requiert]→ `fait_monde_valdore`
  - **pnj_alba** —[requiert]→ `fait_joueur_jardinier`
  - **pnj_alba** —[requiert]→ `lieu_maison_racine`
  - **pnj_alba** —[requiert]→ `lieu_vallee_claire`
  - **pnj_alba** —[requiert]→ `culture_souci_de_la_vallee`
  - **pnj_alba** —[releve_de]→ `system_relations`
  - **pnj_alba** —[donne]→ `quete_le_premier_echo`
  - **pnj_alba** —[donne]→ `quete_ce_qui_se_repete`
...
Produis la **fiche de production** du PNJ `pnj_alba` : identité, fonction,
