...
# PROMPT SPÉCIALISÉ — animation : Puits de Mémoire
...
Séquence `actif` de `machine_puits_de_memoire` — 1 animations :
- `anim_machine_puits_de_memoire_actif` direction=None frames=4 fps=6 loop=True impact=3 effet=effet:memoire_fin

## 5. Relations dans le graphe du monde

- Relations entrantes (ce qui pointe vers l'entité) :
  - `recette_fragment_ravive` —[utilise]→ **machine_puits_de_memoire**
- Relations sortantes (ce vers quoi l'entité pointe) :
  - **machine_puits_de_memoire** —[consomme]→ `objet_echo_cristallin`
  - **machine_puits_de_memoire** —[produit]→ `objet_fragment_de_souvenir`
  - **machine_puits_de_memoire** —[contient]→ `recette_fragment_ravive`
  - **machine_puits_de_memoire** —[releve_de]→ `system_memoire`
...
- `objet_echo_cristallin` (Écho Cristallin)


## 8. Tâche

Produis la **spécification de production** de l'animation `machine_puits_de_memoire`
(action `actif`, direction `toutes (séquence)`). L'animation doit être
...
```json
{
  "id": "machine_puits_de_memoire",
  "entity_id": "<entité propriétaire, imposée>",
  "state_or_action": "actif",
